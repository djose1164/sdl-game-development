                               ...
                               std::vector<GameObject *> *objects)
{
    for (auto e = objectRoot->FirstChildElement(); e; e = e->NextSiblingElement())
    {
        ...
        SDL_Log("[%s]: textureId: %s\nx: %d\ty: %d\twidth:%d\theight: %d\tcallbackId: %d", 
            __func__, textureId.c_str(), x, y, width, height, callbackId);

        auto gameObject{TheGameObjectFactory::instance()->create(
            e->Attribute("type")
        )};

        gameObject->load(std::unique_ptr<LoaderParams>( new LoaderParams(
